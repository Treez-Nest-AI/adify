...
// Alternative API call function if the main one continues to fail
  ...
  const makeApiCall = async (storedData) => {
    try {
      ...
      const response = await axios.post(
        'https://dead11.app.n8n.cloud/webhook/d664dc96-6d9d-419b-aee8-c1cf23dd8fb2/nest-ai/projectbrief',
...
{analysisData.target_audience && (
  <Card className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 hover:shadow-xl transition-shadow duration-300">
    ...
    <div className="space-y-4">
      {/* Fixed Demographics Section */}
      {analysisData.target_audience.demographics && (
        <div className="bg-gradient-to-br from-purple-50 to-blue-50 rounded-xl p-6 border border-purple-100">
          <h4 className="font-bold text-gray-900 mb-3">Demographics</h4>
          {typeof analysisData.target_audience.demographics === 'object' ? (
            <div className="space-y-2">
              {analysisData.target_audience.demographics.gender && (
                <p className="text-gray-600"><span className="font-medium">Gender:</span> {analysisData.target_audience.demographics.gender}</p>
              )}
              {analysisData.target_audience.demographics.age_range && (
                <p className="text-gray-600"><span className="font-medium">Age Range:</span> {analysisData.target_audience.demographics.age_range}</p>
              )}
              {analysisData.target_audience.demographics.location && (
                <p className="text-gray-600"><span className="font-medium">Location:</span> {analysisData.target_audience.demographics.location}</p>
              )}
              {analysisData.target_audience.demographics.income_level && (
                <p className="text-gray-600"><span className="font-medium">Income Level:</span> {analysisData.target_audience.demographics.income_level}</p>
              )}
            </div>
          ) : (
      ...
      {analysisData.target_audience.interests && (
        <div className="bg-gradient-to-br from-blue-50 to-green-50 rounded-xl p-6 border border-blue-100">
          ...
          {Array.isArray(analysisData.target_audience.interests) ? (
            <div className="space-y-1">
              {analysisData.target_audience.interests.map((interest, index) => (
      ...
      {analysisData.target_audience.lifestyle && (
        <div className="bg-gradient-to-br from-green-50 to-purple-50 rounded-xl p-6 border border-green-100">
          ...
          {Array.isArray(analysisData.target_audience.lifestyle) ? (
            <div className="space-y-1">
              {analysisData.target_audience.lifestyle.map((lifestyle, index) => (
                <p key={index} className="text-gray-600">• {lifestyle}</p>
              ))}
            </div>
          ) : (
