...
import { Button } from "@/components/ui/button";
import { useNavigate } from "react-router-dom";
import { Heart, MessageCircle, Share, Upload, X, Check, ArrowLeft } from "lucide-react";
import { useEffect, useRef, useState } from "react";
...
import { 
  ...
  clearLaunchError,clearAllState
...
export default function CampaignStudio() {
  ...
  // Get data from Redux with fallback defaults
  // Import RootState from your store typings
  ...
  const [budget, setBudget] = useState<number>(10); // Start with $10 for 2 ad sets
  const [selectedAdSetNames, setSelectedAdSetNames] = useState<string[]>([]); // Track selected ad set names
  const [selectedAdSetForPreview, setSelectedAdSetForPreview] = useState<AdSet | null>(null);
  const [previewImage, setPreviewImage] = useState<string | null>(null);
  const [uploadedImages, setUploadedImages] = useState<{[key: string]: string}>({}); // Store images per ad set
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [isUploading, setIsUploading] = useState(false);
  const [dragActive, setDragActive] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  // Calculate max selectable ad sets based on budget
  const maxSelectableAdSets = Math.floor(budget / 5);

  // Initialize with first 2 ad sets selected when adSets are loaded
  useEffect(() => {
    if (adSets.length > 0 && selectedAdSetNames.length === 0) {
      const initialSelected = adSets.slice(0, Math.min(2, adSets.length, maxSelectableAdSets))
        .map(adSet => adSet.ad_set_name);
      setSelectedAdSetNames(initialSelected);
      
      // Set first ad set as preview
      if (adSets.length > 0) {
        setSelectedAdSetForPreview(adSets[0]);
      }
    }
  }, [adSets.length, maxSelectableAdSets]);
  ...
  useEffect(() => {
    ...
    if (launchStatus === 'succeeded') {
      toast({
        title: "Campaign Launched Successfully",
        description: "Your campaign has been launched successfully!",
      });
       dispatch(clearAllState()); // Clear all state after successful launch
      navigate('/');
    }

    if (launchStatus === 'failed' && launchError) {
      toast({
  ...
  }, [adSets, status, launchStatus, error, launchError]);
  ...
  const handleBudgetChange = (value: number) => {
    ...
    // If new budget allows fewer selections, trim the selected list
    const newMaxSelectable = Math.floor(clampedValue / 5);
    if (selectedAdSetNames.length > newMaxSelectable) {
      setSelectedAdSetNames(prev => prev.slice(0, newMaxSelectable));
    }
  };

  const handleAdSetSelection = (adSetName: string, isChecked: boolean) => {
    if (isChecked) {
      // Add if we haven't reached the limit
      if (selectedAdSetNames.length < maxSelectableAdSets) {
        setSelectedAdSetNames(prev => [...prev, adSetName]);
      } else {
        toast({
          title: "Selection Limit Reached",
          description: `You can only select ${maxSelectableAdSets} ad sets with your current budget of $${budget}`,
          variant: "destructive",
        });
      }
    } else {
      // Remove from selection
      setSelectedAdSetNames(prev => prev.filter(name => name !== adSetName));
    }
  };

  const handleAdSetPreview = (adSet: AdSet) => {
    setSelectedAdSetForPreview(adSet);
    
    // Load existing uploaded image for this ad set
    if (uploadedImages[adSet.ad_set_name]) {
      // Already has uploaded image, don't change current preview/upload state
    } else {
      // Clear preview state when switching to ad set without image
      setPreviewImage(null);
      setSelectedFile(null);
    }
  };
  ...
  const confirmUpload = async () => {
    if (!selectedFile || !selectedAdSetForPreview) return;
    ...
    try {
      ...
      // Store uploaded image for this specific ad set
      setUploadedImages(prev => ({
        ...prev,
        [selectedAdSetForPreview.ad_set_name]: previewImage!
      }));
      ...
      toast({
        ...
        description: `Image uploaded for ${selectedAdSetForPreview.ad_set_name}`,
      });
  ...
  const removeUploadedImage = (adSetName: string) => {
    const imageUrl = uploadedImages[adSetName];
    if (imageUrl) {
  ...
  const handleLaunchCampaign = async () => {
    ...
    if (!campaignConfig.projectName || !campaignConfig.targetingLocation) {
      toast({
    ...
    if (selectedAdSetNames.length === 0) {
      toast({
        title: "No Ad Sets Selected",
        description: "Please select at least one ad set to launch the campaign.",
        variant: "destructive",
      });
      return;
    }

    // Prepare selected ad sets data
    const selectedAdSetsData: SelectedAdSetData[] = selectedAdSetNames.map(adSetName => {
      const adSet = adSets.find(ads => ads.ad_set_name === adSetName);
      if (!adSet) throw new Error(`Ad set ${adSetName} not found`);
      
      return {
        adSet,
        uploadedImage: uploadedImages[adSetName] || null,
        budget: 5
      };
    });

    // Update Redux with final selected ad sets
    dispatch(updateSelectedAdSets(selectedAdSetsData));

    // Clear any previous launch errors
    dispatch(clearLaunchError());

    // Launch the campaign
    dispatch(launchCampaign({
      campaignConfig,
      selectedAdSets: selectedAdSetsData,
      totalBudget: budget
    }));
  };
  ...
  // No ad sets available
  if (status === 'succeeded' && (!adSets || adSets.length === 0)) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center max-w-md">
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
    ...
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 py-6 max-w-7xl">
        ...
        <div className="mb-6">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-1">Budget Configuration</h3>
                <p className="text-sm text-gray-600">Each ad set costs $5. You can select up to {maxSelectableAdSets} ad sets.</p>
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold text-purple-600">${budget}</div>
              </div>
            ...
            <div className="mt-4 p-3 bg-purple-50 rounded-md">
              <div className="flex justify-between items-center">
                <span className="text-sm text-purple-700">Selected Ad Sets:</span>
                <span className="font-semibold text-purple-800">{selectedAdSetNames.length} of {maxSelectableAdSets} available</span>
              </div>
              <div className="flex justify-between items-center mt-1">
                <span className="text-sm text-purple-700">Total Cost:</span>
                <span className="font-semibold text-purple-800">${selectedAdSetNames.length * 5}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-6">
          ...
          {/* Ad Sets Selection Sidebar */}
          <div className="lg:col-span-3">
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
              <div className="p-4 border-b border-gray-200 bg-gray-50">
                <h3 className="font-semibold text-gray-900">Select Ad Sets</h3>
                <p className="text-xs text-gray-600 mt-1">Choose which ad sets to include in your campaign</p>
              </div>
              
              <div className="max-h-96 overflow-y-auto">
                {adSets.map((adSet, index) => {
                  const isSelected = selectedAdSetNames.includes(adSet.ad_set_name);
                  const isCurrentPreview = selectedAdSetForPreview?.ad_set_name === adSet.ad_set_name;
                  const hasImage = uploadedImages[adSet.ad_set_name];
                  const canSelect = selectedAdSetNames.length < maxSelectableAdSets || isSelected;
                  
                  return (
                    <div
                      key={`${adSet.ad_set_name}-${index}`}
                      className={`p-4 border-b border-gray-100 transition-colors ${
                        isCurrentPreview ? 'bg-blue-50 border-l-4 border-l-blue-500' : 'hover:bg-gray-50'
                      } ${!canSelect && !isSelected ? 'opacity-50' : 'cursor-pointer'}`}
                      onClick={() => handleAdSetPreview(adSet)}
                    >
                      <div className="flex items-start space-x-3">
                        {/* Checkbox */}
                        <div className="pt-0.5">
                          <input
                            ...
                            onChange={(e) => {
                              e.stopPropagation();
                              handleAdSetSelection(adSet.ad_set_name, e.target.checked);
                            }}
          ...
          {/* Ad Set Details Panel */}
          <div className="lg:col-span-5">
            {selectedAdSetForPreview ? (
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-blue-50">
                  <div className="flex items-center justify-between">
                    <div>
                      <h2 className="text-xl font-bold text-gray-900">{selectedAdSetForPreview.ad_set_name}</h2>
                      <p className="text-sm text-gray-600 mt-1">Campaign targeting based on your marketing angles</p>
                    </div>
                    <div className="text-right">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                        selectedAdSetNames.includes(selectedAdSetForPreview.ad_set_name)
                          ? 'bg-green-100 text-green-800'
                          : 'bg-gray-100 text-gray-600'
                      }`}>
                        {selectedAdSetNames.includes(selectedAdSetForPreview.ad_set_name) ? 'Selected' : 'Not Selected'}
                      </span>
                      <div className="text-lg font-bold text-purple-600 mt-1">$5/day</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 rounded-lg p-4">
                      <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Objective</div>
                      <div className="text-sm font-semibold text-gray-900">{selectedAdSetForPreview.campaign_objective}</div>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-4">
                      <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Location</div>
                      <div className="text-sm font-semibold text-gray-900">{selectedAdSetForPreview.target_audience.location}</div>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-4">
                      <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Age Range</div>
                      <div className="text-sm font-semibold text-gray-900">{selectedAdSetForPreview.target_audience.age_range}</div>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-4">
                      <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Gender</div>
                      <div className="text-sm font-semibold text-gray-900">{selectedAdSetForPreview.target_audience.gender}</div>
                    </div>
                  </div>
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-gray-900 mb-3">Target Interests</h4>
                    <div className="flex flex-wrap gap-2">
                      {selectedAdSetForPreview.target_audience.interests.map((interest, idx) => (
                        <span key={idx} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                          {interest}
                        </span>
                      ))}
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-gray-900 mb-3">Ad Placements</h4>
                    <div className="space-y-2">
                      {selectedAdSetForPreview.placements.map((placement, idx) => (
                        <div key={idx} className="flex items-center p-2 bg-gray-50 rounded">
                          {placement.includes('YouTube') && <div className="h-4 w-4 bg-red-600 rounded mr-2" />}
                          {placement.includes('Instagram') && <div className="h-4 w-4 bg-gradient-to-br from-purple-600 to-pink-600 rounded mr-2" />}
                          {placement.includes('Facebook') && <div className="h-4 w-4 bg-blue-600 rounded mr-2" />}
                          {placement.includes('WhatsApp') && <div className="h-4 w-4 bg-green-600 rounded mr-2" />}
                          <span className="text-sm text-gray-700">{placement}</span>
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-gray-900 mb-3">Creative Brief</h4>
                    <p className="text-sm text-gray-700 leading-relaxed bg-gray-50 p-4 rounded-lg">
                      {selectedAdSetForPreview.creative_brief}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Primary Text</div>
                        <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded border-l-4 border-purple-500">
                          {selectedAdSetForPreview.primary_text}
                        </p>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Headline</div>
                          <p className="text-sm font-medium text-gray-900 bg-gray-50 p-3 rounded">{selectedAdSetForPreview.headline}</p>
                        </div>
                        <div>
                          <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Call to Action</div>
                          <button className="bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium px-4 py-2 rounded transition-colors">
                            {selectedAdSetForPreview.call_to_action || campaignConfig.conversionButton || 'Shop Now'}
                          </button>
                        </div>
                      </div>
                      
                      {/* Short Form Variations */}
                      {selectedAdSetForPreview.ad_copy_variations?.short_form?.length > 0 && (
                        <div>
                          <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Short Form Variations</div>
                          <div className="space-y-1">
                            {selectedAdSetForPreview.ad_copy_variations.short_form.map((variation, idx) => (
                              <p key={idx} className="text-sm text-gray-600 bg-gray-50 p-2 rounded">
                                {variation}
                              </p>
                            ))}
                      ...
                      {/* Long Form Variations */}
                      {selectedAdSetForPreview.ad_copy_variations?.long_form?.length > 0 && (
                        <div>
                          <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Long Form Variations</div>
                          <div className="space-y-1">
                            {selectedAdSetForPreview.ad_copy_variations.long_form.map((variation, idx) => (
                              <p key={idx} className="text-sm text-gray-600 bg-gray-50 p-2 rounded">
                                {variation}
                              </p>
                            ))}
              ...
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-8 text-center">
                <p className="text-gray-500">Click on an ad set to view details and preview</p>
              </div>
            )}
          </div>
              ...
              <CardContent className="p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                  <div className={`w-2 h-2 rounded-full mr-2 ${
                    selectedAdSetForPreview && uploadedImages[selectedAdSetForPreview.ad_set_name] ? 'bg-green-400' :
                    previewImage ? 'bg-orange-400' : 'bg-blue-400'
                  } animate-pulse`}></div>
                  {selectedAdSetForPreview && uploadedImages[selectedAdSetForPreview.ad_set_name] ? 'Image Uploaded!' :
                    previewImage ? 'Preview Mode' :
                      'Live Preview with Image Upload'}
                </h3>
                  ...
                  <div className="bg-gray-900 rounded-3xl p-2 shadow-2xl">
                    ...
                    <div className="bg-black rounded-2xl p-1">
                      <div className="bg-white rounded-xl overflow-hidden relative">
                        ...
                        {/* Main Content Area */}
                        <div className="relative" style={{ minHeight: '500px' }}>
                          {/* Default Content (No Image or Preview) */}
                          {!previewImage && (!selectedAdSetForPreview || !uploadedImages[selectedAdSetForPreview.ad_set_name]) && (
                            <div>
                              {/* App Header */}
                              <div className="bg-gradient-to-r from-gray-100 to-gray-50 px-4 py-3 border-b border-gray-200">
                                <div className="flex items-center space-x-3">
                                  ...
                                  <div>
                                    <p className="font-semibold text-gray-900 text-sm">
                                      {selectedAdSetForPreview ? selectedAdSetForPreview.ad_set_name : 'Campaign Studio'}
                                    </p>
                                    <p className="text-gray-500 text-xs">Sponsored</p>
                                  </div>
                                </div>
                              ...
                              <div className="p-4">
                                <p className="text-gray-700 text-sm mb-4 leading-relaxed">
                                  {selectedAdSetForPreview ? selectedAdSetForPreview.primary_text : 'Select an ad set to preview your campaign'}
                                </p>
                                ...
                                <div className="text-center mb-4">
                                  <h4 className="font-bold text-gray-900 text-sm mb-3">
                                    {selectedAdSetForPreview ? selectedAdSetForPreview.headline : 'Campaign Headline'}
                                  </h4>

                                  <Button className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white text-sm">
                                    {selectedAdSetForPreview ? (selectedAdSetForPreview.call_to_action || campaignConfig.conversionButton || 'Shop Now') : 'Call to Action'}
                                  </Button>
                                </div>
                              ...
                              <div className="h-full relative">
                                ...
                                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/80 to-transparent p-4">
                                  <div className="text-white mb-4">
                                    ...
                                    <p className="text-xs opacity-90">
                                      {selectedAdSetForPreview ? selectedAdSetForPreview.primary_text : 'Campaign description'}
                                    </p>
                                  </div>
                          ...
                          {/* Final Uploaded Image */}
                          {selectedAdSetForPreview && uploadedImages[selectedAdSetForPreview.ad_set_name] && (
                            <div className="absolute inset-0">
                              <button
                                onClick={() => removeUploadedImage(selectedAdSetForPreview.ad_set_name)}
                                className="absolute top-2 right-2 z-20 bg-black bg-opacity-50 text-white rounded-full p-2 hover:bg-opacity-70 transition-all"
                              >
                                <X className="h-4 w-4" />
                              </button>
                              ...
                              <div className="h-full relative">
                                <img
                                  src={uploadedImages[selectedAdSetForPreview.ad_set_name]}
                                  alt="Uploaded campaign image"
                                  className="w-full h-full object-cover bg-black"
                                />
                                ...
                                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/70 to-transparent p-4 text-white">
                                  <h4 className="font-bold text-sm mb-2">
                                    {selectedAdSetForPreview.headline}
                                  </h4>
                                  <Button className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white text-sm mb-3">
                                    {selectedAdSetForPreview.call_to_action || campaignConfig.conversionButton || 'Shop Now'}
                                  </Button>
        ...
        <div className="mt-8 flex justify-center space-x-4">
          ...
          <Button 
            className="px-6 py-2 bg-purple-600 hover:bg-purple-700 text-white"
            disabled={selectedAdSetNames.length === 0 || launchStatus === 'loading'}
            onClick={handleLaunchCampaign}
          >
            {launchStatus === 'loading' ? 'Launching...' : `Launch Campaign (${selectedAdSetNames.length} Ad Sets)`}
          </Button>
        </div>
      </div>
    </div>
